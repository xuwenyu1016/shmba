...
//        }
    ...
    public  function  sendmail_teacher_withtel($sendcontent,$tel)
    {
        ...
        $mailto ='';
        //先判断是否存在这个电话的老师
        $sqlcheck = "select * from dux_ext_guestbook where tel= '".$tel."' and emailRemind!='' order by data_id desc";
        $tdata= $model->query($sqlcheck);

        //若全部is_send_over=1 则全部设置成0
        ...
        if ($tdata) {
            # code...
            $mailto =$tdata[0]['emailRemind'];
            if(!strpos($tdata[0]['emailRemind'], '|'))
            {
                $mailto = '';
            }
            else
            {
                $arrmail = explode("|",$tdata[0]['emailRemind']);
                $mailto = $arrmail[1];
                $tname = $arrmail[0].'|'.$mailto;
            }
